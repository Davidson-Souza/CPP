...
#define FOR(k) for(unsigned register int i = 0;i<k; i++)
...
void sha_compress(ctx *shaCtx)
{
    ...
    #endif
    for (unsigned register int i=16; i<=63; i++)
    {
     ...
     **/
    for(unsigned register int i = 0; i<=63; i++)
    {
        s1 = S1();
        ch = Ch();
...
*/
void sha_update(ctx *shaCtx, const unsigned char *block, int size)
{
...
void sha_finalize(struct sha_ctx *ctx, unsigned char ret[32])
{
